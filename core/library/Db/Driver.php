...
abstract class Driver {
    ...
    public function connect($config='',$linkNum=0,$autoConnection=false) {
        if ( !isset($this->linkID[$linkNum]) ) {
            ...
            try{
                ...
                //debug time  2017年4月2日21:41:40
                //result :   charst : utf8 or not utf-8
            ...
            // dump($this->options);
                ...
                if($autoConnection){
                    LogRecord('[ Driver.class.php connect() line:101 ERR ]'.$e->getMessage());
        ...
        return isset($this->linkID[$linkNum]) ? $this->linkID[$linkNum] : $this->linkID;
    }
    ...
    public function error() {
        ...
        // 记录错误日志
        LogRecord('[ '.__FILE__.' error() line:348 ERR]'.$this->error);
    ...
    protected function parseWhereItem($key,$val) {
        ...
        if(is_array($val)) {
            if(is_string($val[0])) {
                ...
                if(isset($val[$count-1])){
                    if(is_array($val[$count-1])){
                        $rule = strtoupper($val[$count-1][0]);
                    }else{
                        $rule = strtoupper($val[$count-1]);
    ...
    protected function debug($start) {
        if($this->config['debug']) {// 开启数据库调试模式
            if($start) {
                ...
                // 记录操作结束时间
                G('queryEndTime');
                LogRecord($this->queryStr.' [ SQL RunTime:'.G('queryStartTime','queryEndTime').'s ]');
            }
        }
    }
