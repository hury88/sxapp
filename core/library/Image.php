...
缩略图
$image = R('Image');
$image->open('./1.jpg');// 生成一个居中裁剪为150*150的缩略图并保存为thumb.jpg
$image->thumb(150, 150,Image::IMAGE_THUMB_CENTER)->save('./thumb.jpg');

裁切图
$image = R('Image');
支持从某个坐标开始裁剪，例如下面从（100，30）开始裁剪：
$image->open('./1.jpg');//将图片裁剪为400x400并保存为corp.jpg
$image->crop(400, 400,100,30)->save('./crop.jpg');

给图片加水印
$image->open('./1.jpg');//将图片裁剪为440x440并保存为corp.jpg
$image->crop(440, 440)->save('./crop.jpg');// 给裁剪后的图片添加图片水印（水印文件位于./logo.png），位置为右下角，保存为water.gif
    ...
    public function __construct($type = self::IMAGE_GD, $imgname = null){
        ...
        switch ($type) {
            ...
            case self::IMAGE_IMAGICK:
                ...
                break;
            default:
                E(LogRecord('不支持的图片处理库类型'));
        }

        /* 引入处理库，实例化图片处理对象 */
        include_once("Image/Driver/{$class}.class.php");
