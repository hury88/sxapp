   ...
   public $token;
    ...
    private function get_access_token()
    {
        ...
        if(NS_VERSION == NS_VER_B2C || NS_VERSION == NS_VER_B2C_FX)
        {
            ...
            //开放平台模式获取access_token
            $weixin_open = new WchatOpen();
            $token = $weixin_open->get_access_token($this->author_appid);
            if(!empty($token))
            {
    ...
    private function single_get_access_token(){
        ...
        $url = 'https://api.weixin.qq.com/cgi-bin/token?grant_type=client_credential&appid='.$wchat_config['oauth_value']['appid'].'&secret='.$wchat_config['oauth_value']['appsecret'];
        $ch = curl_init();
        ...
        if (strpos($_SERVER['HTTP_USER_AGENT'], 'MicroMessenger')) {
            ...
            if (empty($_GET['code'])){
                ...
                if(NS_VERSION == NS_VER_B2C || NS_VERSION == NS_VER_B2C_FX)
                {
                    ...
                    $weixin_open = new WchatOpen();
                    $url = $weixin_open->get_authorize_url_info($this->author_appid, $baseUrl, "123");
                }
                Header("Location: $url");
                exit();
            }  else {
                ...
                if(NS_VERSION == NS_VER_B2C || NS_VERSION == NS_VER_B2C_FX)
                {
                    ...
                    $weixin_open = new WchatOpen();
                    $data = $weixin_open->get_access_token_member($this->author_appid,$code);
                    return $data;
                }
            }
    ...
    public function get_single_access_token($code = ''){
        $config = new Config();
        $wchat_config = $config->getInstanceWchatConfig(0);
        $token_url = 'https://api.weixin.qq.com/sns/oauth2/access_token?appid='.$wchat_config['oauth_value']['appid'].'&secret='.$wchat_config['oauth_value']['appsecret'].'&code='.$code.'&grant_type=authorization_code';
    ...
    public function get_single_authorize_url($redirect_url = '', $state = ''){
        ...
        $config = new Config();
        $wchat_config = $config->getInstanceWchatConfig(0);
        return "https://open.weixin.qq.com/connect/oauth2/authorize?appid=".$wchat_config['oauth_value']['appid']."&redirect_uri=".$redirect_url."&response_type=code&scope=snsapi_userinfo&state={$state}#wechat_redirect";
    }
    /**
     * 获取会员对于公众号信息
     * @param unknown $appid
