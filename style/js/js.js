...
$(".inputpic2").click(function() {
	$("#inputpic2").show();
	$(".blackbox").show();
})
$("#genderWindowClose").click(function() {
	$("#inputsex").hide();
	$(".blackbox").hide();
})
...
$("#ym").click(function() {
	if($(this).hasClass("addym")) {
		...
		$(".yanma").attr('type', 'text');
		// $(".xianma").hide();

	} else {

		$(".yanma").attr('type', 'password');
		// $(".yanma").hide();
		// $(".xianma").show();
...
/*$(document).ready(function() {
	var add, reduce, num, num_txt, dom_goods_number;
	add = $(".J_jia"); //添加数量
	reduce = $(".J_jian"); //减少数量
	num = ""; //数量初始值
	num_txt = $(".num"); //接受数量的文本框
	dom_goods_number = $(".snum"); //商品数量

	var stock = $("#d_stock").val()*1;
	...
	add.click(function() {
		...
		if (num < stock) {
			...
			num++;
			goods_number = dom_goods_number.text() * 1;
			dom_goods_number.text(++goods_number);
	...
	reduce.click(function() {
		...
		if(num > 0) {
			...
			if(num == 1) {
				num--;
				$(this).next("label").children(".num").val("1");
			}
			//否则就执行减减方法
			else {
				num--;
